...
}, movie.name = "kathmandu_custom1.gif", interval = 1.0, outdir = getwd())
```
![](kathmandu_custom1.gif)
...
saveGIF({ 
    time_lapse(kathmandu_small, 'year', 
               ...
               size=2, alpha=.2)
}, movie.name = "kathmandu_custom2.gif", interval = 1.0, outdir = getwd())
```
![](kathmandu_custom2.gif)
...
To do this, we'll need to:
  * calculate the `timeunit` attribute, a rounded version of `timestamp`. This is normally done by the `time_lapse` function, which are bypassing.
...
}, movie.name = "kathmandu_custom.gif", interval = 1.0, outdir = getwd())
```
![](kathmandu_custom.gif)
...
If you do build something cool, please do post it on the [Examples page](https://github.com/prabhasp/OSMTimeLapseR/wiki/Example-visualizations)