...
plotMe <- function(week_start, pp, debug=F) {
  ...
  untilnow <- subset(pp, unixDay < toUnixDay(week_end))
  p <- ggplot() + 
    geom_map(data = untilnow, aes(map_id = id, fill = unixDay >= toUnixDay(week_start)),
             map = untilnow) + expand_limits(x = pp$long, y = pp$lat) +
